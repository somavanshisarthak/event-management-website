...
router.post('/', auth, checkRole(['organizer', 'admin']), async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO events (title, description, date_time, capacity, organizer_id) VALUES (?, ?, ?, ?, ?)',
      [title, description, date_time, capacity, organizer_id]
    );

    res.status(201).json({
      message: 'Event created successfully',
      event: { id: result.insertId, title, description, date_time, capacity, organizer_id }
    });
  } catch (err) {
